...
Что бы сделать pull request надо:
* Необходимо найти в левом меню пункт Pull requests (Github) и воспользоваться кнопкой Create a pull request.
* На следующей странице необходимо указать с какого репозитория и с какой ветки на какой репозиторий и на какую ветку будет осуществляться пулл-реквест.
* Также необходимо заполнить title в поле ниже и нажать на кнопку Create a pull request.
